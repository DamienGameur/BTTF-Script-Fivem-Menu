...
-- local fx = 'ent_amb_beach_campfire' -- head
    ...
    for _, bike in ipairs(current) do
      if not ptfxs[bike] then
        ...
        for _, offset in ipairs(GetWheelOffsets(bike)) do
          ...
          local ring = CreateParticle(bike, 'ent_amb_beach_campfire', ringOffset, 3.0)
          table.insert(ptfxs[bike], ring)

          for i=0, 3 do
            local fire = CreateParticle(bike, 'ent_amb_beach_campfire', fireOffset, 3.0)
